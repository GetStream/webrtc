...
class DefaultKeyProviderImpl : public KeyProvider {
 public:
  DefaultKeyProviderImpl(KeyProviderOptions* options) {
    ...
    // NOTE: GetStream/webrtc @ m145 has no KeyProviderOptions::
    // key_derivation_algorithm field (and no webrtc::KeyDerivationAlgorithm
    // enum), so this option is not propagated; core uses its default key
    // derivation. Consistent with the fork's mobile SDKs.
